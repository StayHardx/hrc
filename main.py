...
async def send_email(email, subject, html_content):
    ...
    sender_password = "Neriah2024@"  # Replace
    ...
    try:
        ...
        with smtplib.SMTP_SSL(
            "smtp.mail.eu-west-1.awsapps.com", 465, context=context
        ) as server:
...
async def delayed_email(email, name):
    await asyncio.sleep(600)  # Wait for 1 hour (3600 seconds)
...
@app.post("/send-email")
async def handle_email_task(request: LongRequest):
...
async def handle_direct_email(request: LongRequest):
